module SkillsSimilitary
  def calculate_skills_similitary(position_skills, recruiter_skills)
    ...
    cosine_similarity(skills_proficiency.map { |_k, v| v[:position] }, skills_proficiency.map { |_k, v| v[:recruiter] })
  end
  ...
  def count_skills_proficiency(skills, skills_counters, required_skills, key)
    skills.each do |skill|
      next unless required_skills.include?(skill['skill'])

      skills_counters[skill['skill']][key] = skill['proficiency']
    end
    skills_counters
  end
  ...
  def cosine_similarity(vector1, vector2)
    ...
    dot / (Math.sqrt(denom_a) * Math.sqrt(denom_b))
  end
